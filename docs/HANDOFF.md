...
- 배포 완료: SHA `0120fe4` 로 웹·워커·스케줄러를 모두 교체했다
  (`deploy_worker=true`). 용량 상한(T0=96MB)은 그대로 두었다 — 호스트 RAM 이
  1.9GB 다.
- 배포 후 검증: `counter.flush` 의 completed 기록을 4분 간격으로 추적해
  29 → 61 까지 분당 1건씩 오른 뒤 **60~61 에서 평형**에 드는 것을 확인했다
  (16:08~17:00). `age: 3600` 과 분당 1회 주기가 만나는 예측값과 같다. 전체 키도
  241 → 237 로 돌아섰다. 수정 전이라면 같은 자리에서 2주간 28,690건까지 갔다.
...
- 기본 흐름은 관련 로컬 테스트·정적 검사 → `main` push → 서버 fetch/pull 및 웹 반영 →
  `/api/ready`·대상 화면·컨테이너 health 확인이다.
- 웹 변경은 `DEPLOY_WORKER=0`으로 웹만 교체한다. 이미지 잡은 패키징 수단이며 전체 CI
  통과로 기록하지 않는다.
- 인증·권한·개인정보·DB 파괴 변경·인프라·워커는 축소 검증 대상이 아니다. 생략한 검사는
  인수인계에 남긴다.
- 운영 상세와 서버 직접 빌드의 자원 안전 조건은 `20_OPS/O01_DEPLOY.md` §1-0을 따른다.
- 전체 CI 필수화는 출시 후보(RC) 고정 또는 사용자의 별도 지시 때 다시 활성화한다.

## 2026-08-27 가입·이메일 인증 UX 최신 배포

- 운영 웹 SHA는 `fbacf6bb912eafae8e4c061e991b3e3c7575efcd`다.
- 가입은 계정 → 프로필 → 이용 설정 3단계이며, 국가 아래에 필수/선택 동의를 함께 둔다.
- 이메일은 한 줄만 입력하고 예시 전용 도메인은 거부한다.
- 인증 성공은 가입 탭과 인증 탭 모두 자동으로 로그인 화면으로 이어지며, 만료 링크는
  같은 인증 화면에서 재발송할 수 있다.
