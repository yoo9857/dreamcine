...
위 표의 값은 **서버가 감당하는 총량**이다. 그것을 사용자별로 나누는 축이
`UserRole` × `MemberTier` 이고, 계산은 `resolveEntitlements()` 하나가 한다.

| 등급 | 일일 업로드 배분 | 시간당 세션 | 영상 길이 | 시리즈 수 |
|---|---|---|---|---|
| `BRONZE` | 20% | 2회 | 50% | 5 |
| `SILVER` | 40% | 3회 | 75% | 20 |
| `GOLD` | 60% | 5회 | 100% | 60 |
| `PLATINUM` | 80% | 8회 | 100% | 120 |
| `DIAMOND` | 100% | 12회 | 100% | 200 |

규칙 4개:

1. **등급은 용량을 늘리지 않고 배분한다.** 늘리는 구조로 만들면 등급 인플레가
   그대로 서버 부하가 된다. 어떤 등급도 티어 표의 값을 넘지 못한다.
2. **파일 1개 최대 용량은 등급으로 나누지 않는다.** 그것은 워커의 임시공간과
   타임아웃에서 나온 기술적 한계이지 혜택이 아니다.
3. **제작 권한이 없는 역할은 배분이 0 이다.** `GUEST` `VIEWER` `MEMBER`
   `MODERATOR` 는 등급이 `DIAMOND` 여도 업로드 한도가 0 이다.
4. **`PARTNER` 는 하한을 보장받는다** (일일 80% · 시간당 8회 · 길이 100% ·
   시리즈 120). 정산 대상이 등급 등락에 흔들리면 계약이 성립하지 않는다.
   `ADMIN` 은 배분 대상이 아니고 용량 전부를 쓴다 — 운영 검증 업로드가 등급에
   막히면 장애 대응이 막힌다.

시간당 세션은 배분 값과 티어 값 중 **작은 쪽**을 쓴다. T0 의 시간당 5회 아래에서
`DIAMOND` 의 12회는 5회로 잘린다 — 표의 12는 T1 이상에서 의미를 갖는다.
