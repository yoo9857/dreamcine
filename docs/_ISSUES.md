...
- 결정: 승인 — `@lhci/cli@0.15.1`만 루트 devDependency로 고정하고 의존성 계약 검사에
  근거를 등록한다. 프로덕션 의존성이나 웹 번들에는 포함하지 않는다. (2026-08-25, 사용자 승인)
- 상태: APPROVED
...
- 결정: 권장안 채택. 에피소드별 1시간 서명 HttpOnly 쿠키를 사용하고, 생년 입력은
  판정 즉시 폐기해 DB·쿠키·로그에 저장하지 않는다. (2026-08-24, 사용자 승인)
- 상태: RESOLVED
...
- 결정: 권장안 채택. 인증 선택 `POST /api/episodes/:id/views`를 추가하고 식별자는
  서버가 세션 또는 HMAC IP로만 만든다. (2026-08-24, 사용자 승인)
- 상태: RESOLVED
...
## [ISS-021] 영상 유지율·접속 지역을 계산할 재생 이벤트 원장 계약이 없다
- 발견 단계: 관리자 영상 분석 확장
- 스펙 위치: `04_DOMAIN_MODEL.md` (`Episode.avgWatchSec`, `WatchProgress`) · `05_API_CONTRACT.md` (`progress`, `views`) · `06_MEDIA_PIPELINE.md` §5
- 문제: 현재 저장되는 분석 관련 값은 누적 조회수, 평균 시청 초, 로그인 사용자의 마지막 이어보기 위치뿐이다. 이 값만으로는 일별 조회 추이, 순 시청자, 0–100% 구간별 유지율, 재시청 스파이크, 주요 이탈, 최근 48시간, 실제 접속 국가, 유입 경로, 기기, 버퍼링·오류율을 재구성할 수 없다. 사용자 프로필의 `country`는 접속 국가가 아니며 `WatchProgress.positionSec`는 세션별 시청 구간이 아니다.
- 사용자 요구 (2026-08-27): 조회수·시청 구간·유지율을 YouTube급으로 설계하고 월드맵 기반 지역 분석을 구현한다.
- 필요한 계약:
  1. `SESSION_START`, `PLAY`, `PAUSE`, `HEARTBEAT`, `SEEK`, `BUFFER`, `ERROR`, `END` append-only 이벤트. `sessionId + sequence` 유니크로 beacon 재전송을 멱등 처리한다.
  2. 세션 차원: episode, 익명 시청자 키, 실제 시청 초, 완료 여부, 유입 경로, 기기, 화질, CDN/프록시가 제공한 ISO 3166-1 접속 국가. 원본 IP는 저장하지 않는다.
  3. 날짜×영상×국가×기기×유입 경로 일별 집계와 영상×0–100 유지율 버킷. 현재 시청자는 Redis 45초 TTL로 분리한다.
  4. 유지율은 구간 시청 횟수/유효 재생 세션으로 계산해 재시청 시 100%를 넘을 수 있게 하고, 완주율은 실제 시청 구간 95% 이상을 기준으로 한다.
  5. 개인정보: 국가 셀 순 시청자 10명 미만 숨김, 원본 이벤트 90일·일별 집계 25개월 제안, 알 수 없는 국가는 `ZZ`, 도시는 기본 미수집.
- 현재 구현: `/admin/analytics`는 기존 DB에서 정직하게 계산 가능한 누적 조회·평균 시청·영상별 성과만 실데이터로 표시한다. 이벤트가 필요한 패널은 빈 상태다. `/admin-preview/analytics`만 그래프와 월드맵 검증용이라고 명시한 미리보기 데이터를 사용한다.
- 영향: 승인 전에는 재생 이벤트 테이블·수집 API·플레이어 하트비트 확장을 추가하지 않는다. UI와 조회 모델은 이벤트 계약이 확정되면 동일한 인터페이스로 연결할 수 있다.
- 상태: OPEN (재생 이벤트·보존·지역 개인정보 계약 승인 대기)

## [OBS-022] 월드맵 분석 렌더러로 MapLibre GL JS를 추가한다
- 발견 단계: 관리자 영상 분석 확장
- 사용자 요구 (2026-08-27): 접속 지역을 확대·축소·툴팁이 가능한 월드맵 애널리틱스로 구현한다.
- 결정: `maplibre-gl` 6.6.0을 분석 페이지의 시청자 탭에서만 동적 로드한다. 공항 전체 데이터는 영상 분석 도메인과 무관하므로 포함하지 않고, 국가별 집계 좌표와 비중 마커만 렌더링한다.
- 번들 제약: MapLibre GL JS 6의 공식 Next.js 안내에 따라 `maplibre-gl-worker.mjs`와 `maplibre-gl-shared.mjs`를 `public/maplibre`에 함께 복사하고 same-origin worker를 사용한다.
- 외부 연결: 사용자 제공 Carto 기본 지도의 스타일·벡터 타일·글리프·스프라이트에 필요한 `basemaps.cartocdn.com`, `tiles.basemaps.cartocdn.com` 및 타일 분산 서브도메인만 CSP `connect-src`에 허용한다.
- 데이터 한계: 실서비스 국가 집계는 ISS-021의 재생 이벤트 계약 승인 전까지 빈 상태이며, 지도 샘플 마커는 개발 전용 미리보기에서만 사용한다.
- 상태: RESOLVED (UI 렌더러 및 빌드 설정)
