...
- [x] E2E 실검증 — CI 런 32539989821 (2026-08-22) / 테마·CSP nonce·보안헤더·키보드 완주 통과.
      테마 E2E 가 한 번 실패했으나 제품 결함이 아니었다 (OBS-008) — 테마 쿠키는 프로덕션
      빌드에서 `Secure` 가 붙고, Chrome 은 localhost 예외로 http 에서도 보내지만 Playwright 의
      `APIRequestContext` 는 규칙대로 보내지 않는다. 확인하려는 것이 "서버가 보낸 HTML" 이므로
      실제 탐색의 응답 본문을 읽도록 고쳤다.
