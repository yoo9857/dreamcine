...
- [x] S3 구현 — 2026-08-22 / 잔존 NIE 0 · 단위 126건 · 통합 18건(CI) · `gate:s2`·`gate:contract` PASS

### S3 구현 순서 변경

마커 13 `T04:storageErrors` 를 **가장 먼저** 구현했다. 나머지 전부가 에러
매핑에 의존하므로, 마지막에 붙이면 그 전까지의 S3 호출은 원시 에러를 그대로
흘리게 된다 — 중간 상태가 잘못된 동작을 하는 순서는 피한다.
