...
### 1-0. 현재 개발 기간의 임시 Fast lane (2026-08-27 사용자 결정)

출시 후보를 고정하기 전까지는 개발 속도를 우선한다. 이 기간에는 아래 흐름을 기본으로
사용하며, **전체 GitHub Actions gate의 완료를 기다리는 것을 일반 배포의 선행 조건으로
삼지 않는다.** 이 결정은 아래 §1의 출시·안정화 원칙에 대한 기간 한정 예외다.

```text
변경 → 관련 테스트 + lint/typecheck → git commit/push
     → 운영 서버 git fetch/pull → 웹 빌드/교체 → readiness와 화면 스모크
```

- 평범한 웹 UI·문구·비파괴 API 변경은 관련 테스트와 정적 검사만 통과하면 바로 배포한다.
- 웹 변경은 `DEPLOY_WORKER=0`으로 웹만 교체한다. 워커·스케줄러를 불필요하게 재시작하지
  않는다.
- GitHub Actions의 이미지 잡만 사용한 경우에도 이를 “전체 CI 통과”라고 기록하지 않는다.
  이미지 잡은 배포물 패키징 수단일 뿐이다.
- 운영 서버 직접 빌드는 `df -h`, `free -h`, 현재 readiness, 실행 중 트랜스코드 유무를
  먼저 확인한다. 자원이 부족하거나 미디어 잡이 실행 중이면 서버 빌드 대신 이미지 잡을
  사용한다. 서비스 중인 T0/T1 호스트의 OOM은 개발 속도보다 손실이 크다.
- DB 파괴적 변경, 인증·권한, 결제, 개인정보, 백업/복구, Compose·Caddy, 워커 변경은
  Fast lane 대상이 아니다. 해당 변경은 최소 계약·통합 검사와 배포 전 백업을 수행한다.
- 매 배포 후 `/api/ready`, 대상 화면, 컨테이너 health를 확인한다. 실패하면 새 수정을
  이어 붙이지 말고 직전 SHA로 롤백한다.
- 전체 CI를 필수 관문으로 되돌리는 시점은 **출시 후보(RC) 고정 또는 사용자의 별도
  지시 시점**이다. 그때 §1·§2의 정식 파이프라인을 다시 기본값으로 사용한다.

다음 작업자는 CI 실패 수정만을 위해 개발 흐름을 멈추지 않는다. 현재 변경과 직접 관련된
로컬 검사가 통과했고 운영 스모크가 가능한 경우 Fast lane으로 진행하고, 생략한 전체
검사는 인수인계에 정확히 기록한다.
