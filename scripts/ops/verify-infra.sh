...
verify_dev() {
  local postgres_id redis_id minio_id cdn_id status originals_status public_status headers
  local hls_status thumbs_status

  cleanup_cors_probe() {
    docker compose -f "${COMPOSE_DEV}" run --rm --no-deps --entrypoint /bin/sh minio-init -c \
      'mc alias set local "$MINIO_ENDPOINT" "$MINIO_ROOT_USER" "$MINIO_ROOT_PASSWORD" >/dev/null && mc rm --force "local/$S3_BUCKET_HLS/hls/cors-probe" "local/$S3_BUCKET_THUMBS/thumbs/cdn-probe" >/dev/null' \
      >/dev/null 2>&1 || true
  }
  ...
  cdn_id="$(docker compose -f "${COMPOSE_DEV}" ps -q cdn)"
  [[ -n "${postgres_id}" && -n "${redis_id}" && -n "${minio_id}" && -n "${cdn_id}" ]] || fail 'dev containers are not running'
  ...
  status="$(docker inspect --format '{{.State.Health.Status}}' "${cdn_id}")"
  [[ "${status}" == 'healthy' ]] || fail "cdn is ${status}"
  ...
  docker compose -f "${COMPOSE_DEV}" run --rm --no-deps --entrypoint /bin/sh minio-init -c 'mc alias set local "$MINIO_ENDPOINT" "$MINIO_ROOT_USER" "$MINIO_ROOT_PASSWORD" >/dev/null && printf hls | mc pipe "local/$S3_BUCKET_HLS/hls/cors-probe" >/dev/null && printf thumb | mc pipe "local/$S3_BUCKET_THUMBS/thumbs/cdn-probe" >/dev/null'
  hls_status="$(curl -sS -o /dev/null -w '%{http_code}' 'http://127.0.0.1:9002/hls/cors-probe')"
  [[ "${hls_status}" == '200' ]] || fail "CDN HLS route returned ${hls_status}, expected 200"
  thumbs_status="$(curl -sS -o /dev/null -w '%{http_code}' 'http://127.0.0.1:9002/thumbs/cdn-probe')"
  [[ "${thumbs_status}" == '200' ]] || fail "CDN thumbs route returned ${thumbs_status}, expected 200"
  headers="$(curl -sS -D - -o /dev/null -H 'Origin: http://localhost:3000' 'http://127.0.0.1:9002/hls/cors-probe')"
  ...
  cleanup_cors_probe
  trap - EXIT
  printf 'PASS: dev PostgreSQL, Redis, MinIO policies, CDN routes and CORS\n'
}
